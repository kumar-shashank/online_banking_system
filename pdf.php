...
$query=$conn->query( "select * from passbook".$sender_id."",$conn);
while($data=mysqli_fetch_array($query)){


	$pdf->Cell(20,5,$data[0],1,0);
	$pdf->Cell(50,5,$data[1],1,0);
	$pdf->Cell(50,5,$data[8],1,0);
	$pdf->Cell(20,5,$data[5],1,0);
	$pdf->Cell(20,5,$data[6],1,0);
	$pdf->Cell(39,5,$data[7],1,0);
	
	if($pdf->GetStringWidth($data[8]) > 65){
		$pdf->SetFont('Arial','',7);
		$pdf->Cell(65,5,$data[8],'LR',0);
		$pdf->SetFont('Arial','',9);
	}else{
		$pdf->Cell(65,5,$data[8],'LR',0);
	}
	$pdf->Cell(60,5,$data[7],'LR',1);
}
$pdf->Output();
?>
